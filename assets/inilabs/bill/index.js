...
jQuery(document).ready(function() {
  ...
  jQuery(document).on('click', '#billAdd', function(event) {
    ...
    if(patientID === '0') {
      error++;
      jQuery('#uhid').parent().addClass('text-danger').parent().children('.select2-container').addClass('is-invalid');
      jQuery('#uhid-error').text('The UHID field is required.');
    } else {
      jQuery('#uhid').parent().removeClass('text-danger').parent().children('.select2-container').removeClass('is-invalid');
  ...
  function ajaxCall(passData)
  {
    jQuery.ajax({
      type: 'POST',
      url: THEMEBASEURL+'bill/savebill',
      data: passData,
      async: true,
      dataType: 'html',
  ...
  function errorLoader(response)
  {
    if(response.status) {
      ...
      jQuery('#billAdd').removeAttr('disabled');
  ...
  jQuery(document).on('change', '#billcategoryID', function() {
    ...
    if(billcategoryID === '0') {
      jQuery('#billlabelID').html('<option value="0">— '+bill_please_select_lang+' —</option>');
    } else {
      jQuery.ajax({
        type: 'POST',
  ...
  function productItemDesign(productID, productText)
  {
    ...
    if(jQuery('#billList tr:last').text() === '') {
      ...
      let lastTdNumber = jQuery('#billList tr:last td:eq(0)').text();
    }
