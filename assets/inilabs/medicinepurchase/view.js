jQuery(document).on('click', '#sendpdf', function() {
  ...
  let medicinepurchaseID = myMedicinepurchaseID;
  let error = 0;
  ...
  if(error === 0) {
    ...
    jQuery.ajax({
      type: 'POST',
      url: THEMEBASEURL+'medicinepurchase/sendmail',
      data: {'to': to, 'subject': subject, 'message': message, 'medicinepurchaseID': medicinepurchaseID, [CSRFNAME] : CSRFHASH},
      dataType: 'html',
