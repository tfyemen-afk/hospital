jQuery(document).ready(function () {
  ...
  jQuery('#payment_amount').on('keyup', function () {
    ...
    if (dotAndNumber(productsalepaidamount)) {
      ...
      productsalepaidamount = parseSentenceForNumber(jQuery(this).val());
  ...
  jQuery(document).on('click', '#addMedicineButton', function (event) {
    ...
    let medicineitems = jQuery('tr[id^=tr_]').map(function (index, value) {
      return {
        ...
        'randid' : jQuery(this).attr('randid'),
    ...
    jQuery.ajax({
      ...
      success : function (data) {
        ...
        if (error === 0) {
          jQuery.ajax({
            type : 'POST',
            url : THEMEBASEURL + 'medicinepurchase/check_validation',
            data : {
            ...
            success : function (data) {
              ...
              if (error === 0) {
                ...
                formData.append('medicinepurchaseID', medicinepurchaseID);
  ...
  function errorLoader(response) {
    if (response.status) {
      window.location = THEMEBASEURL + 'medicinepurchase/index/' + displayID;
    } else {
  ...
  jQuery(document).on('change', '#medicineID', function() {
    ...
    if(medicineID > 0) {
      ...
      jQuery('#medicineList').find('.datepicker').datepicker({format: 'dd-mm-yyyy', startDate: set_start_expire_date, autoclose:true});
    }
  });

  jQuery('.datepicker').datepicker({format: 'dd-mm-yyyy', startDate: set_start_expire_date, autoclose:true});
  ...
  let globalsubtotal = totalAmount;
  function totalInfo() {
    let totalQuantity = 0;
    let totalSubtotal = 0;
    jQuery('#medicineList tr').each(function(index, value) {
  ...
  jQuery(document).on('keyup', '.change-medicinequantity', function() {
    let salequantity       = parseInt(jQuery(this).data('salequantity'));
    let medicinequantity   = toFixedVal(jQuery(this).val());
    if(salequantity > medicinequantity) {
      medicinequantity   = salequantity;
