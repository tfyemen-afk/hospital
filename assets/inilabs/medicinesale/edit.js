jQuery(document).ready(function() {
  ...
  let patient_type = my_patient_type;
  if(patient_type === 2) {
  ...
  jQuery(document).on('click', '#addMedicineButton', function(event) {
    ...
    jQuery.ajax({
      ...
      data: {'medicineitems': medicineitems,'update':'update', 'medicinesaleID':myMedicinesaleID, [CSRFNAME] : CSRFHASH},
      dataType: 'html',
      success: function(data) {
        ...
        if(error === 0) {
          ...
          formData.append('medicinesaleID', myMedicinesaleID);
  ...
  function errorLoader(response) {
    if(response.status) {
      window.location = THEMEBASEURL+'medicinesale/index/'+myDisplayID;
    } else {
  ...
  jQuery(document).on('change', '#medicineID', function() {
    ...
    if(medicineID > 0) {
      jQuery.ajax({
        ...
        data: {'medicineID': medicineID, 'update':'update', 'medicinesaleID': myMedicinesaleID, [CSRFNAME] : CSRFHASH},
        dataType: 'html',
        success: function(data) {
          let response = JSON.parse(data);
          if(response.status) {
  ...
  function medicineItemDesign(medicineID, medicineName, batchs) {
    ...
    text += '<option value="0">'+myMedicinesale_please_select+'</option>';
  ...
  let globalsubtotal = myMedicinesaletotalamount;
  function totalInfo() {
    let totalQuantity = 0;
    let totalSubtotal = 0;
    jQuery('#medicineList tr').each(function(index, value) {
  ...
  jQuery(document).on('keyup', '.change-medicinequantity', function() {
    ...
    if(dotAndNumber(medicineQuantity)) {
      ...
      let medicineQuantity = parseSentenceForNumber(toFixedVal(jQuery(this).val()));
