...
jQuery(document).on('click', '#sendpdf', function() {
  ...
  let medicinesaleID = myMedicinesaleID;
  let error = 0;
  ...
  if(error === 0) {
    ...
    jQuery.ajax({
      type: 'POST',
      url: THEMEBASEURL+'medicinesale/sendmail',
      data: {'to': to, 'subject': subject, 'message': message, 'medicinesaleID': medicinesaleID, [CSRFNAME] : CSRFHASH},
      dataType: 'html',
