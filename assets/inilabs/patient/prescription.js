jQuery(document).on('click', '#sendpdf', function () {
  ...
  let appointmentAndAdmissionTypeID = myAppointmentAndAdmissionTypeID;
  let appointmentAndAdmissionID = myAppointmentAndAdmissionID;
  let patientID = myPatientID;
  let error = 0;
  ...
  if (error === 0) {
    ...
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'patient/prescriptionsendmail',
      data : {
        'to' : to,
        'subject' : subject,
        'message' : message,
        'appointmentAndAdmissionTypeID' : appointmentAndAdmissionTypeID,
