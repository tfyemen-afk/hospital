jQuery(document).ready(function () {
  ...
  jQuery(document).on('change', '#roleID', function () {
    ...
    if (roleID > 0) {
      jQuery.ajax({
        type : 'POST',
        url : THEMEBASEURL + 'salaryreport/get_user',
  ...
  jQuery(document).on('change', '#userID, #month, #from_date, #to_date', function () {
    jQuery('#load_salaryreport').html('');
  });

  jQuery(document).on('click', '#get_salaryreport', function () {
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'salaryreport/getsalaryreport',
      data : passData,
      dataType : 'html',
      success : function (data) {
  ...
  function renderLoder(response, passData) {
    if (response.status) {
      jQuery('#load_salaryreport').html(response.render);
