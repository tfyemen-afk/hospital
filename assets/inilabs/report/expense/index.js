jQuery(document).ready(function () {
  ...
  jQuery(document).on('change', '#from_date', function () {
    jQuery('#load_expensereport').html('');
  });
  ...
  jQuery(document).on('click', '#get_expensereport', function () {
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'expensereport/getexpensereport',
      data : passData,
      dataType : 'html',
  ...
  function renderLoder(response, passData) {
    if (response.status) {
      jQuery('#load_expensereport').html(response.render);
