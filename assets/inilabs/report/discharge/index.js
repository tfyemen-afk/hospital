jQuery(document).ready(function () {
  ...
  jQuery(document).on('change', '#conditionofdischarge, #from_date, #to_date', function () {
    jQuery('#load_dischargereport').html('');
  });

  jQuery(document).on('click', '#get_dischargereport', function () {
    let conditionofdischarge = jQuery('#conditionofdischarge').val();
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'dischargereport/getdischargereport',
      data : passData,
      dataType : 'html',
  ...
  function renderLoader(response, passData) {
    if (response.status) {
      jQuery('#load_dischargereport').html(response.render);
