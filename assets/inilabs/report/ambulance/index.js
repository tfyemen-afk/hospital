jQuery(document).ready(function() {
  ...
  jQuery(function() {
    jQuery('#ambulanceID').val(0);
  });

  jQuery(document).on('change', '#ambulanceID, #from_date, #to_date', function() {
    jQuery('#load_ambulancereport').html('');
  });

  jQuery(document).on('click','#get_ambulancereport', function() {
    let ambulanceID= jQuery('#ambulanceID').val();
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'ambulancereport/getambulancereport',
      data : passData,
      dataType : 'html',
  ...
  function renderLoder(response, passData) {
    if(response.status) {
      jQuery('#load_ambulancereport').html(response.render);
