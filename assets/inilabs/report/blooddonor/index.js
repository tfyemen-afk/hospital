jQuery(document).ready(function () {
  ...
  jQuery(document).on('change', '#bloodgroupID, #blooddonorID, #patientID, #statusID, #bagno, #from_date, #to_date', function() {
    jQuery('#load_blooddonorreport').html('');
  });

  jQuery(document).on('click','#get_blooddonorreport', function() {
    let bloodgroupID = jQuery('#bloodgroupID').val();
    let blooddonorID = jQuery('#blooddonorID').val();
    let patientID    = jQuery('#patientID').val();
    let statusID     = jQuery('#statusID').val();
    let bagno        = jQuery('#bagno').val();
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type:'POST',
      url: THEMEBASEURL+'blooddonorreport/getblooddonorreport',
      data:passData,
      dataType:'html',
  ...
  function renderLoder(response, passData) {
    if(response.status) {
      jQuery('#load_blooddonorreport').html(response.render);
