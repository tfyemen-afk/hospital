jQuery(document).ready(function () {
  ...
  jQuery(document).on('change', '#tpaID, #typeID, #from_date, #to_date', function () {
    jQuery('#load_tpareport').html('');
  });

  jQuery(document).on('click', '#get_tpareport', function () {
    let tpaID = jQuery('#tpaID').val();
    let typeID = jQuery('#typeID').val();
  ...
  function ajaxCall(passData) {
    jQuery.ajax({
      type : 'POST',
      url : THEMEBASEURL + 'tpareport/gettpareport',
      data : passData,
      dataType : 'html',
  ...
  function renderLoder(response, passData) {
    if (response.status) {
      jQuery('#load_tpareport').html(response.render);
