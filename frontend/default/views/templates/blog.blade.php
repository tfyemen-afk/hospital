                	...
                	<div class="blog-section">
                		@if(inicompute($posts))
                			@foreach($posts as $post)
		                    	<article class="blog">
		                    		<header>
		                    			<a href="{{ site_url('frontend/post/'.$post->url) }}">
		                    				{{ $post->title }}
		                    			</a> 
		                    		</header>
		                    		<div>Posted on {{ date('dS F, Y', strtotime($post->publish_date)) }} by <?=inicompute(getUser($post->create_userID)) ? getUser($post->create_userID)->name : '' ?></div>
		                    		<div class="blog-body">
		                    			...
		                    			<p>
		                    				@if(strlen(strip_tags($post->content)) > 250)
		                    					{{ namesorting(strip_tags($post->content), 250) }} <a href="{{ site_url('frontend/post/'.$post->url) }}"> Read More » </a>
		                    				@else
                ...
                <div class="col-xs-12 col-sm-4">
                    <div class="blog-recennt-post">
                    	...
                    	@if(inicompute($posts))
                    		<?php $i=1; ?> 
                    		@foreach($posts as $post)
                    			<div class="post-title"><a href="{{ site_url('frontend/post/'.$post->url) }}"> <i class="fa fa-arrow-right"></i> {{ namesorting(strip_tags($post->title), 75) }}</a></div>
                    			@if($i == 6)
                    				<?php break; ?>
                    			@endif
                    			<?php $i++; ?>
