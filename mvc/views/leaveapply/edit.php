        ...
        <div class="col-sm-8">
            <nav class="breadcrumb-position" aria-label="breadcrumb">
                <ol class="breadcrumb themebreadcrumb pull-right">
                    ...
                    <li class="breadcrumb-item"><a href="<?=site_url('leaveapply/index')?>"><?=$this->lang->line('menu_leaveapply')?></a></li>
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('leaveapply_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                        ...
                        <div class="card-block">
                            <div class="form-group <?=form_error('roleID') ? 'text-danger' : '' ?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('roleID', $roleArray,  set_value('roleID', $leaveapply->applicationto_roleID), ' class="form-control select2 '.$errorClass.'" id="roleID" ');
                                ?>
                            ...
                            <div class="form-group <?=form_error('applicationto_userID') ? 'text-danger' : '' ?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('applicationto_userID', $userArray,  set_value('applicationto_userID', $leaveapply->applicationto_userID), ' class="form-control select2 '.$errorClass.'" id="applicationto_userID" ');
                                ?>
                            ...
                            <div class="form-group <?=form_error('leavecategoryID') ? 'text-danger' : '' ?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('leavecategoryID', $leaveassignArray,  set_value('leavecategoryID', $leaveapply->leavecategoryID), ' class="form-control select2 '.$errorClass.'" ');
                                ?>
                            ...
                            <div class="form-group <?=form_error('schedule') ? 'text-danger' : '' ?>">
                                ...
                                <?php 
                                    $schedule = date('m/d/Y', strtotime($leaveapply->from_date)).' - '.date('m/d/Y', strtotime($leaveapply->to_date));
                                ?>
                                <input type="text" class="form-control daterange <?=form_error('schedule') ? 'is-invalid' : '' ?>" id="schedule" name="schedule"  value="<?=set_value('schedule', $schedule)?>">
                            ...
                            <div class="form-group <?=form_error('reason') ? 'text-danger' : '' ?>">
                                ...
                                <textarea name="reason" id="reason" class="form-control <?=form_error('reason') ? 'is-invalid' : '' ?>" cols="30" rows="5"><?=set_value('reason', $leaveapply->reason)?></textarea>
                            ...
                            <div class="form-group <?=form_error('od_status') ? 'text-danger' : '' ?>">
                                ...
                                <input type="checkbox" id="od_status" name="od_status" <?=($leaveapply->od_status == 1) ? 'checked' :''?> value="1" <?=set_checkbox('od_status', '1'); ?>>
                                <span><?=form_error('od_status')?></span>
                            </div>
                        </div>
                        <div class="card-footer"> 
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('leaveapply_update')?></button>
                        </div>
                    </form>
                </div>
            </div>
