<article class="content">
        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"> <?=$this->lang->line('menu_ward')?></li>
                </ol>
            </nav>
        </div>
    </div>
    <section class="section">
        <div class="row">
            <?php if(permissionChecker('ward_add')) { ?>
                <div class="col-sm-4">
                        ...
                        <form role="form" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-block">
                                ...
                                <div class="form-group <?=form_error('floorID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="floorID"><?=$this->lang->line('ward_floor')?><span class="text-danger"> *</span></label>
                                     <?php
                                        ...
                                        echo form_dropdown('floorID', $floorArray,  set_value('floorID'), ' class="form-control select2 '.$errorClass.'"'); ?>
                                ...
                                <div class="form-group <?=form_error('roomID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="roomID"><?=$this->lang->line('ward_room')?><span class="text-danger"> *</span></label>
                                     <?php
                                        ...
                                        echo form_dropdown('roomID', $roomArray,  set_value('roomID'), ' class="form-control select2 '.$errorClass.'"'); ?>
                                ...
                                <div class="form-group <?=form_error('description') ? 'text-danger' : '' ?>">
                                    ...
                                    <textarea name="description" id="description" class="form-control <?=form_error('description') ? 'is-invalid' : '' ?>" cols="30" rows="5"><?=set_value('description')?></textarea>
                                    <span><?=form_error('description')?></span>
                                </div>
                            </div>
                            <div class="card-footer"> 
                                <button type="submit" class="btn btn-primary"><?=$this->lang->line('ward_add')?></button>
                            </div>
                        </form>
                    </div>
                </div>
            <?php } ?>
            <div class="<?=(permissionChecker('ward_add') ? 'col-sm-8' : 'col-sm-12')?>">
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="header-block">
                            <p class="title"><i class="fa fa-table"></i>&nbsp;<?=$this->lang->line('ward_list')?></p>
                        </div>
                    </div>
                    <div class="card-block">
