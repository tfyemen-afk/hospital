        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="<?=site_url('itemcheckin/index')?>"><?=$this->lang->line('menu_itemcheckin')?></a></li>
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('itemcheckin_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                        ...
                        <div class="card-block">
                            ...
                            <div class="form-group <?=form_error('itemID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="itemID"><?=$this->lang->line('itemcheckin_item')?><span class="text-danger"> *</span></label>
                                    ...
                                    echo form_dropdown('itemID', $itemArray,  set_value('itemID', $itemcheckin->itemID), ' id="itemID" class="form-control select2 '.$errorClass.'"')?>
                            ...
                            <div class="form-group <?=form_error('supplierID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="supplierID"><?=$this->lang->line('itemcheckin_supplier')?><span class="text-danger"> *</span></label>
                                    ...
                                    echo form_dropdown('supplierID', $itemsupplierArray,  set_value('supplierID', $itemcheckin->supplierID), ' class="form-control select2 '.$errorClass.'"')?>
                            ...
                            <div class="form-group <?=form_error('storeID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="storeID"><?=$this->lang->line('itemcheckin_store')?><span class="text-danger"> *</span></label>
                                    ...
                                    echo form_dropdown('storeID', $storeArray,  set_value('storeID', $itemcheckin->storeID), ' class="form-control select2 '.$errorClass.'"')?>
                            ...
                            <div class="form-group <?=form_error('quantity') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('quantity') ? 'is-invalid' : '' ?>" id="quantity" name="quantity"  value="<?=set_value('quantity', $itemcheckin->quantity)?>">
                            ...
                            <div class="form-group <?=form_error('date') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" autocomplete="off" class="form-control <?=form_error('date') ? 'is-invalid' : '' ?>" id="date" name="date"  value="<?=set_value('date', date('d-m-Y H:i A', strtotime($itemcheckin->date)))?>">
                            ...
                            <div class="form-group <?=form_error('description') ? 'text-danger' : '' ?>">
                                ...
                                <textarea type="text" class="form-control <?=form_error('description') ? 'is-invalid' : '' ?>" id="description" name="description" rows="3"><?=set_value('description', $itemcheckin->description)?></textarea>
                                <span><?=form_error('description')?></span>
                            </div>
                        </div>
                        <div class="card-footer"> 
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('itemcheckin_update')?></button>
                        </div>
                    </form>
                </div>
            </div>
