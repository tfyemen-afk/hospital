<article class="content">
        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb pull-right themebreadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('menu_ambulancecall')?></li>
              </ol>
            </nav>
        </div>
    </div>
    <section class="section">
        <div class="row">
            <?php if(permissionChecker('ambulancecall_add')) { ?>
                <div class="col-sm-4">
                            ...
                            <div class="card-block">
                                <div class="form-group <?=form_error('ambulanceID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="ambulanceID"><?=$this->lang->line('ambulancecall_ambulance')?> <span class="text-danger">*</span></label>
                                        ...
                                        echo form_dropdown('ambulanceID', $ambulanceArray,  set_value('ambulanceID'), ' id="ambulanceID" class="form-control select2 '.$errorClass.'"')?>
                                ...
                                <div class="form-group <?=form_error('date') ? 'text-danger' : '' ?>">
                                    ...
                                    <input autocomplete="off" type="text" class="form-control datepicker <?=form_error('date') ? 'is-invalid' : '' ?>" id="date" name="date"  value="<?=set_value('date')?>">
                                ...
                                <div class="form-group <?=form_error('patientname') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('patientname') ? 'is-invalid' : '' ?>" id="patientname" name="patientname"  value="<?=set_value('patientname')?>">
                                ...
                                <div class="form-group <?=form_error('patientcontact') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('patientcontact') ? 'is-invalid' : '' ?>" id="patientcontact" name="patientcontact"  value="<?=set_value('patientcontact')?>">
                                ...
                                <div class="form-group <?=form_error('pickup_point') ? 'text-danger' : '' ?>">
                                    ...
                                    <textarea type="text" class="form-control <?=form_error('pickup_point') ? 'is-invalid' : '' ?>" id="pickup_point" name="pickup_point" rows="3"><?=set_value('pickup_point')?></textarea>
                                    <span><?=form_error('pickup_point')?></span>
                                </div>
                            </div>
                            <div class="card-footer"> 
                                <button type="submit" class="btn btn-primary"><?=$this->lang->line('ambulancecall_add')?></button>
                            </div>
                        </form>
                    </div>
                </div>
            <?php } ?>
            <div class="<?=(permissionChecker('ambulancecall_add') ? 'col-sm-8' : 'col-sm-12')?>">
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="header-block">
