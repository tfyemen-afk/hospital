    ...
    $(function() {
            ...
            $('#earningGraph').highcharts({
                ...
                series: [{
                    ...
                    data: [
                        <?php
                            foreach ($months as $key => $month) {
                                if(isset($income_month_total[$key])) {
                                    echo "{y:".$income_month_total[$key].", monthID:'".lzero($key)."', monthName:'".$month."', 'dayWiseData': '".json_encode($income_month_day_total[$key])."', 'type': 'income'},";
                                } else {
                    ...
                    data: [
                        <?php
                            foreach ($months as $key => $month) {
                                if(isset($expense_month_total[$key])) {
                                    echo "{y:".$expense_month_total[$key].", monthID:'".$key."', monthName:'".$month."', 'dayWiseData': '".json_encode($expense_month_day_total[$key])."', 'type': 'expense'},";
                                } else {
        ...
        function LoadDayWiseExpenseOrIncome(type, monthID, monthName, dayWiseData) {
          'use strict';
            ...
            $.ajax({
                type: 'POST',
                url: "<?=site_url('dashboard/dayWiseExpenseOrIncome')?>",
                data: {"dayWiseData" : dayWiseData, 'type': type, 'monthID': monthID, 'monthName': monthName, [csrfName] : csrfHash},
                dataType: "html",
