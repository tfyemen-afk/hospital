        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="<?=site_url('item/index')?>"><?=$this->lang->line('menu_item')?></a></li>
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('item_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <form role="form" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="card-block">
                            <div class="form-group <?=form_error('name') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('name') ? 'is-invalid' : '' ?>" id="name" name="name"  value="<?=set_value('name', $item->name)?>">
                            ...
                            <div class="form-group <?=form_error('categoryID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="categoryID"><?=$this->lang->line('item_category')?><span class="text-danger"> *</span></label>
                                    ...
                                    echo form_dropdown('categoryID', $categoryArray,  set_value('categoryID', $item->categoryID), ' class="form-control select2 '.$errorClass.'"')?>
                            ...
                            <div class="form-group <?=form_error('description') ? 'text-danger' : '' ?>">
                                ...
                                <textarea type="text" class="form-control <?=form_error('description') ? 'is-invalid' : '' ?>" id="description" name="description" rows="3"><?=set_value('description', $item->description)?></textarea>
                                <span><?=form_error('description')?></span>
                            </div>
                        </div>
                        <div class="card-footer"> 
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('item_update')?></button>
                        </div>
                    </form>
                </div>
            </div>
