        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('menu_menulog')?></li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <form role="form" method="POST">
                        ...
                        <div class="card-block">
                            ...
                            <div class="form-group <?=form_error('parentmenulogID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="parentmenulogID"><?=$this->lang->line('menulog_parent')?></label>
                                    ...
                                    if(inicompute($menulogs)) {
                                        foreach ($menulogs as $menulog) {
                                            $menuparentArray[$menulog->menulogID] = $menulog->name;
                                        }
                                    }
                                    $errorClass = form_error('parentmenulogID') ? 'is-invalid' : '';
                                    echo form_dropdown('parentmenulogID', $menuparentArray,  set_value('parentmenulogID'), ' class="form-control select2 '.$errorClass.'"')?>
                            ...
                            <div class="form-group <?=form_error('link') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('link') ? 'is-invalid' : '' ?>" id="link" name="link"  value="<?=set_value('link')?>">
                            ...
                            <div class="form-group <?=form_error('icon') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('icon') ? 'is-invalid' : '' ?>" id="icon" name="icon"  value="<?=set_value('icon')?>">
                            ...
                            <div class="form-group <?=form_error('priority') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('priority') ? 'is-invalid' : '' ?>" id="priority" name="priority"  value="<?=set_value('priority')?>">
                            ...
                            <div class="form-group <?=form_error('pullright') ? 'text-danger' : '' ?>">
                                ...
                                <input type="text" class="form-control <?=form_error('pullright') ? 'is-invalid' : '' ?>" id="pullright" name="pullright"  value="<?=set_value('pullright')?>">
                                <span><?=form_error('pullright')?></span>
                            </div>
                        </div>
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('menulog_add')?></button>
                        </div>
                    </form>
                </div>
            </div>
