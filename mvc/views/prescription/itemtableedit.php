            ...
            <div class="box-body box-profile box-border">
                ...
                <ul class="list-group list-group-unbordered">
                    <li class="list-group-item list-group-item-background">
                        <b><?=$this->lang->line('prescription_visit_no')?></b> <a class="pull-right"><?=$prescription->visitno?></a>
                    </li>
                    <li class="list-group-item list-group-item-background">
                    ...
                    <?php if(permissionChecker('patient_view')) { ?>
                        <li class="list-group-item list-group-item-background">
                            <a target="_blank" href="<?=site_url('patient/view/'.$viewpatientID)?>" class="btn btn-primary btn-sm btn-block"><?=$this->lang->line('prescription_details')?></a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
...
<div class="col-sm-8">
    <div class="row">
        ...
        <div class="col-sm-12">
            ...
            <table class="table table-margin table-striped table-bordered table-hover simple-table">
                <thead>
                <tr>
                ...
                <?php if($counter > 0) { $j = 0; for($i = 1; $i <= $counter; $i++) { ?>
                    <tr id="tr_<?=$i?>" trID="<?=$i?>">
                        <td>
                            <?=$i?>
                        </td>
                        <td id="medicine_<?=$i?>">
                            <?=set_value('medicine_'.$i, $prescriptionitems[$j]->medicine)?>
                            <input class="display-none" name="medicine_<?=$i?>" type="text" value="<?=set_value('medicine_'.$i, $prescriptionitems[$j]->medicine)?>">
                        </td>

                        <td id="instruction_<?=$i?>">
                            <?=set_value('instruction_'.$i, $prescriptionitems[$j]->instruction)?>
                            <input class="display-none" name="instruction_<?=$i?>" type="text" value="<?=set_value('instruction_'.$i, $prescriptionitems[$j]->instruction)?>">
                        </td>
