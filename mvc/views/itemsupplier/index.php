<article class="content">
        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb pull-right themebreadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('menu_itemsupplier')?></li>
              </ol>
            </nav>
        </div>
    </div>
    <section class="section">
        <div class="row">
            <?php if(permissionChecker('itemsupplier_add')) { ?>
                <div class="col-sm-4">
                        ...
                        <form role="form" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-block">
                                <div class="form-group <?=form_error('companyname') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('companyname') ? 'is-invalid' : '' ?>" id="companyname" name="companyname"  value="<?=set_value('companyname')?>">
                                ...
                                <div class="form-group <?=form_error('suppliername') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('suppliername') ? 'is-invalid' : '' ?>" id="suppliername" name="suppliername"  value="<?=set_value('suppliername')?>">
                                ...
                                <div class="form-group <?=form_error('address') ? 'text-danger' : '' ?>">
                                    ...
                                    <textarea type="text" class="form-control <?=form_error('address') ? 'is-invalid' : '' ?>" id="address" name="address" rows="3"><?=set_value('address')?></textarea>
                                    <span><?=form_error('address')?></span>
                                </div>
                            </div>
                            <div class="card-footer"> 
                                <button type="submit" class="btn btn-primary"><?=$this->lang->line('itemsupplier_add')?></button>
                            </div>
                        </form>
                    </div>
                </div>
            <?php } ?>
            <div class="<?=(permissionChecker('itemsupplier_add') ? 'col-sm-8' : 'col-sm-12')?>">
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="header-block">
