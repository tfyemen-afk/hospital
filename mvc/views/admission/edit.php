        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="<?=site_url('admission/index/'.$displayDateStrtotime.'/'.$displayDoctorID)?>"> <?=$this->lang->line('menu_admission')?></a></li>
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('admission_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <form role="form" method="POST">
                        ...
                        <div class="card-block">
                            <?php if($this->data['loginroleID'] != 3) { ?>
                                <div class="form-group <?=form_error('uhid') ? 'text-danger' : ''?>">
                                    ...
                                    <?php
                                        ...
                                        echo form_dropdown('uhid', $patientArray,  set_value('uhid', $admission->patientID), 'id="uhid" class="form-control select2 '.$errorClass.'"');
                                    ?>
                            ...
                            <div class="form-group <?=form_error('admissiondate') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control datepicker <?=form_error('admissiondate') ? 'is-invalid' : ''?>" name="admissiondate" id="admissiondate" autocomplete="off" value="<?=set_value('admissiondate', date('d-m-Y h:i A', strtotime($admission->admissiondate)))?>" readonly/>
                            ...
                            <div class="form-group <?=form_error('case') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control <?=form_error('case') ? 'is-invalid' : ''?>" name="case" id="case" value="<?=set_value('case', $admission->pcase)?>"/>
                            ...
                            <div class="form-group <?=form_error('casualty') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('casualty', $casualtyArray,  set_value('casualty', $admission->casualty), 'id="casualty" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('oldpatient') ? 'text-danger' : ''?>">
                                ...
                                echo form_dropdown('oldpatient', $visitedArray,  set_value('oldpatient', $admission->oldpatient), 'id="oldpatient" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <?php if($this->data['loginroleID'] != 3) { ?>
                                <div class="form-group <?=form_error('tpaID') ? 'text-danger' : ''?>">
                                    ...
                                    <?php
                                        ...
                                        echo form_dropdown('tpaID', $tpaArray,  set_value('tpaID', $admission->tpaID), 'id="tpaID" class="form-control select2 '.$errorClass.'"');
                                    ?>
                            ...
                            <div class="form-group <?=form_error('reference') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control <?=form_error('reference') ? 'is-invalid' : ''?>" name="reference" id="reference" value="<?=set_value('reference', $admission->reference)?>"/>
                            ...
                            <div class="form-group <?=form_error('symptoms') ? 'text-danger' : ''?>">
                                ...
                                <textarea id="symptoms" name="symptoms" class="form-control <?=form_error('symptoms') ? 'is-invalid' : ''?>"><?=set_value('symptoms', $admission->symptoms)?></textarea>
                            ...
                            <div class="form-group <?=form_error('allergies') ? 'text-danger' : ''?>">
                                ...
                                <textarea id="allergies" name="allergies" class="form-control <?=form_error('allergies') ? 'is-invalid' : ''?>"><?=set_value('allergies', $admission->allergies)?></textarea>
                                ...
                                <?php
                                    ...
                                    if(inicompute($doctors)) {
                                        foreach ($doctors as $doctor) {
                                            if(($doctor->status == 1 && $doctor->delete_at == 0) || ($doctor->userID == $admission->doctorID)) {
                                                $doctorArray[$doctor->userID] = $doctor->name;
                                            }
                                        }
                                    }
                                    $errorClass = form_error('doctorID') ? 'is-invalid' : '';
                                    echo form_dropdown('doctorID', $doctorArray,  set_value('doctorID', $admission->doctorID), 'id="doctorID" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <?php if($this->data['loginroleID'] != 3) { ?>
                                <div class="form-group <?=form_error('creditlimit') ? 'text-danger' : ''?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('creditlimit') ? 'is-invalid' : ''?>" name="creditlimit" id="creditlimit" value="<?=set_value('creditlimit', $admission->creditlimit)?>"/>
                            ...
                            <div class="form-group <?=form_error('wardID') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('wardID', $wardArray,  set_value('wardID', $admission->wardID), 'id="wardID" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('bedID') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    if(inicompute($beds)) {
                                        foreach ($beds as $bed) {
                                            if($bed->status == 0 || $admission->bedID == $bed->bedID) {
                                    ...
                                    echo form_dropdown('bedID', $bedArray,  set_value('bedID', $admission->bedID), 'id="bedID" class="form-control select2 '.$errorClass.'"');
                                ?>
                                <span><?=form_error('bedID')?></span>
                            </div>
                        </div>
                        <div class="card-footer">
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('admission_update')?></button>
                        </div>
                    </form>
                </div>
            </div>
