        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('appointment_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                    ...
                    <form role="form" method="POST" id="appointmentForm">
                        ...
                        <div class="card-block">
                            <?php if($loginroleID != 3) { ?>
                                <div class="form-group <?=form_error('uhid') ? 'text-danger' : ''?>">
                                    ...
                                    <?php
                                        ...
                                        if(inicompute($patients)) {
                                            foreach ($patients as $patient) {
                                                if($loginroleID != 3) {
                                                    ...
                                                    if($loginuserID == $patient->patientID) {
                                                        $patientArray[$patient->patientID] = $patient->patientID.' - '.$patient->name;
                                                    }
                                                }
                                            }
                                        ...
                                        echo form_dropdown('uhid', $patientArray,  set_value('uhid', $appointment->patientID), 'id="uhid" class="form-control select2 '.$errorClass.'"');
                                    ?>
                            ...
                            <div class="form-group <?=form_error('appointmentdate') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control datepicker <?=form_error('appointmentdate') ? 'is-invalid' : ''?>" name="appointmentdate" id="appointmentdate" autocomplete="off" value="<?=set_value('appointmentdate',date('d-m-Y h:i A',strtotime($appointment->appointmentdate)))?>" readonly/>
                            ...
                            <div class="form-group <?=form_error('case') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control <?=form_error('case') ? 'is-invalid' : ''?>" name="case" id="case" value="<?=set_value('case',$appointment->pcase)?>"/>
                            ...
                            <div class="form-group <?=form_error('casualty') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('casualty', $casualtyArray,  set_value('casualty',$appointment->casualty), 'id="casualty" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('oldpatient') ? 'text-danger' : ''?>">
                                ...
                                echo form_dropdown('oldpatient', $visitedArray,  set_value('oldpatient', $appointment->oldpatient), 'id="oldpatient" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <?php if($loginroleID != 3) { ?>
                                <div class="form-group <?=form_error('tpaID') ? 'text-danger' : ''?>">
                                    ...
                                    <?php
                                        ...
                                        echo form_dropdown('tpaID', $tpaArray,  set_value('tpaID',$appointment->tpaID), 'id="tpaID" class="form-control select2 '.$errorClass.'"');
                                    ?>
                            ...
                            <div class="form-group <?=form_error('reference') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control <?=form_error('reference') ? 'is-invalid' : ''?>" name="reference" id="reference" value="<?=set_value('reference',$appointment->reference)?>"/>
                            ...
                            <div class="form-group <?=form_error('symptoms') ? 'text-danger' : ''?>">
                                ...
                                <textarea id="symptoms" name="symptoms" class="form-control <?=form_error('symptoms') ? 'is-invalid' : ''?>"><?=set_value('symptoms', $appointment->symptoms)?></textarea>
                            ...
                            <div class="form-group <?=form_error('allergies') ? 'text-danger' : ''?>">
                                ...
                                <textarea id="allergies" name="allergies" class="form-control <?=form_error('allergies') ? 'is-invalid' : ''?>"><?=set_value('allergies', $appointment->allergies)?></textarea>
                                <span><?=form_error('allergies')?></span>
                            </div>

                            <?php 
                            $showEdit = true;
                            if($loginroleID == 3 && $paymentstatus == 1) {
                                $showEdit = false;
                            }
                            if($showEdit) { ?>
                            ...
                            <div class="form-group <?=form_error('doctorID') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('doctorID', $doctorArray,  set_value('doctorID',$appointment->doctorID), 'id="doctorID" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('type') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('type', $typeArray,  set_value('type', $appointment->type), 'id="type" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('amount') ? 'text-danger' : ''?>">
                                ...
                                <input type="text" class="form-control <?=form_error('amount') ? 'is-invalid' : ''?>" name="amount" id="amount" value="<?=set_value('amount', $appointment->amount)?>" readonly/>
                            ...
                            <div class="form-group <?=form_error('paymentstatus') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('paymentstatus', $paymentStatus,  set_value('paymentstatus', $paymentstatus), 'id="paymentstatus" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div id="paymentmethodDiv" class="form-group <?=form_error('paymentmethodID') ? 'text-danger' : ''?>">
                                ...
                                <?php
                                    $paymentmethodValue = isset($paymentmethodIDArray[$appointment->paymentmethodID]) ? $paymentmethodIDArray[$appointment->paymentmethodID] : '';
                                    $errorClass = form_error('paymentmethodID') ? 'is-invalid' : '';
                                    echo form_dropdown('paymentmethodID', $paymentmethodArray,  set_value('paymentmethodID',$paymentmethodValue), 'id="paymentmethodID" class="form-control select2 '.$errorClass.'"');
                                ?>
                        ...
                        <div class="card-footer"> 
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('appointment_update')?></button>
                        </div> 
                    </form>
                </div>
            </div>
