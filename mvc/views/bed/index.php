<article class="content">
        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('menu_bed')?></li>
                </ol>
            </nav>
        </div>
    </div>
    <section class="section">
        <div class="row">
            <?php if(permissionChecker('bed_add')) { ?>
                <div class="col-sm-4">
                                ...
                                <div class="form-group <?=form_error('bedtypeID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="bedtypeID"><?=$this->lang->line('bed_bedtype')?><span class="text-danger"> *</span></label>
                                     <?php
                                        ...
                                        echo form_dropdown('bedtypeID', $bedtypeArray,  set_value('bedtypeID'), ' class="form-control select2 '.$errorClass.'"'); ?>
                                ...
                                <div class="form-group <?=form_error('wardID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="wardID"><?=$this->lang->line('bed_ward')?><span class="text-danger"> *</span></label>
                                     <?php
                                        ...
                                        echo form_dropdown('wardID', $wardArray,  set_value('wardID'), ' class="form-control select2 '.$errorClass.'"'); ?>
                                    <span><?=form_error('wardID')?></span>
                                </div>
                            </div>
                            <div class="card-footer"> 
                                <button type="submit" class="btn btn-primary"><?=$this->lang->line('bed_add')?></button>
                            </div>
                        </form>
                    </div>
                </div>
            <?php } ?>
            <div class="<?=(permissionChecker('bed_add') ? 'col-sm-8' : 'col-sm-12')?>">
                <div class="card card-custom">
                    <div class="card-header">
                        <div class="header-block">
