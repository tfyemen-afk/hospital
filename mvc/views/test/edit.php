        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb pull-right themebreadcrumb">
                    ...
                    <li class="breadcrumb-item" aria-current="page"><a href="<?=site_url('test/index/'.$displayID)?>"><?=$this->lang->line('menu_test')?></a></li>
                    <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('test_edit')?></li>
                </ol>
            </nav>
        </div>
    </div>
                        ...
                        <div class="card-block">
                            <div class="form-group <?=form_error('billID') ? 'text-danger' : '' ?>">
                                ...
                                <?php
                                    ...
                                    echo form_dropdown('billID', $billArray,  set_value('billID', $test->billID), ' id="billID" class="form-control select2 '.$errorClass.'"');
                                ?>
                            ...
                            <div class="form-group <?=form_error('testcategoryID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="testcategoryID"><?=$this->lang->line('test_category')?><span class="text-danger"> *</span></label>
                                 <?php
                                    ...
                                    echo form_dropdown('testcategoryID', $testcategoryArray,  set_value('testcategoryID', $test->testcategoryID), ' id="testcategoryID" class="form-control select2 '.$errorClass.'"'); ?>
                            ...
                            <div class="form-group <?=form_error('testlabelID') ? 'text-danger' : '' ?>">
                                <label class="control-label" for="testlabelID"><?=$this->lang->line('test_label')?><span class="text-danger"> *</span></label>
                                 <?php
                                    ...
                                    echo form_dropdown('testlabelID', $testlabelArray,  set_value('testlabelID', $test->testlabelID), ' id="testlabelID" class="form-control select2 '.$errorClass.'"'); ?>
                            ...
                            <div class="form-group <?=form_error('testnote') ? 'text-danger' : '' ?>">
                                ...
                                <textarea name="testnote" class="form-control <?=$errorClass?>" id="testnote" rows="5"><?=set_value('testnote', $test->testnote)?></textarea>
                                <span><?=form_error('testnote')?></span>
                            </div>
                        </div>
                        <div class="card-footer"> 
                            <button type="submit" class="btn btn-primary"><?=$this->lang->line('test_update')?></button>
                        </div>
                    </form>
                </div>
            </div>
