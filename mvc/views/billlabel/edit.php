        ...
        <div class="col-sm-8 pull-right">
            <nav aria-label="breadcrumb">
              <ol class="breadcrumb pull-right themebreadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?=site_url('billlabel/index')?>"> <?=$this->lang->line('menu_billlabel')?></a></li>
                <li class="breadcrumb-item active" aria-current="page"><?=$this->lang->line('billlabel_edit')?></li>
              </ol>
            </nav>
        </div>
    </div>
                        ...
                        <form role="form" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-block">
                                <div class="form-group <?=form_error('name') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('name') ? 'is-invalid' : '' ?>" id="name" name="name"  value="<?=set_value('name', $billlabel->name)?>">
                                ...
                                <div class="form-group <?=form_error('billcategoryID') ? 'text-danger' : '' ?>">
                                    <label class="control-label" for="billcategoryID"><?=$this->lang->line('billlabel_bill_category')?><span class="text-danger"> *</span></label>
                                        ...
                                        echo form_dropdown('billcategoryID', $billcategoryArray,  set_value('billcategoryID', $billlabel->billcategoryID), ' class="form-control select2 '.$errorClass.'"')?>
                                ...
                                <div class="form-group <?=form_error('discount') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('discount') ? 'is-invalid' : '' ?>" id="discount" name="discount"  value="<?=set_value('discount', $billlabel->discount)?>">
                                ...
                                <div class="form-group <?=form_error('amount') ? 'text-danger' : '' ?>">
                                    ...
                                    <input type="text" class="form-control <?=form_error('amount') ? 'is-invalid' : '' ?>" id="amount" name="amount"  value="<?=set_value('amount', $billlabel->amount)?>">
                                    <span><?=form_error('amount')?></span>
                                </div>
                            </div>
                            <div class="card-footer"> 
                                <button type="submit" class="btn btn-primary"><?=$this->lang->line('billlabel_update')?></button>
                            </div>
                        </form>
                    </div>
                </div>
