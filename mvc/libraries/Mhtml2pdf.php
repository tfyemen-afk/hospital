...
class Mhtml2pdf {
	 ...
	 * @return	void
	 */
	function create_normal($mode = 'view', $title, $stylesheet = NULL)
	{
        $mpdf = new Mpdf();
        // $mpdf->baseScript = 1;
    ...
    function create_advance($mode = 'view', $title, $stylesheet = NULL, $headerDesign = NULL, $footerDesign = NULL, $designnone = 1)
    {
        $mpdf = new Mpdf();

        if($designnone) {
            if($headerDesign) {
                $mpdf->SetHTMLHeader($headerDesign);
            }
            if($footerDesign) {
                $mpdf->SetHTMLFooter($footerDesign);
            }
        }
        ...
        $mpdf->AddPage($this->orientation, // L - landscape, P - portrait
                    ...
                    12, // margin_left
                    12, // margin right
                    ($designnone) ? 45 : 10, // margin top
                    ($designnone) ? 30 : 15, // margin bottom
                    5, // margin header
                    5); // margin footer
